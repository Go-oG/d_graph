...
///                                         ^^ middle index
class FastSelect {
  static void fastSelect<T>(List<T> arr, int k,
      [int left = 0, int? right, int Function(T a, T b)? compare]) {
    if (left < 0) {
      left = 0;
    }
    right ??= arr.length - 1;
  ...
  static _fastSelectStep<T>(
      List<T> arr, int k, int left, int right, int Function(T a, T b) compare) {
    while (right > left) {
      if (right - left > 600) {
        int n = right - left + 1;
        int m = k - left + 1;
